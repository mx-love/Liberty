...
let danmakuDensityReduced = false;
...
function startMemoryMonitor() {
    ...
    memoryCheckInterval = setInterval(() => {
        try {
            ...
            console.log(`💾 内存占用: ${usedMemory.toFixed(0)} MB (${usagePercent.toFixed(1)}%)`);
            
            // 策略1: 内存使用超过 60% 时，减少弹幕数量
            if (usagePercent > 60 && !danmakuDensityReduced) {
                console.warn('⚠️ 内存使用较高，减少弹幕密度');
                reduceDanmakuDensity();
                danmakuDensityReduced = true;
            }
            
            // 策略2: 内存使用超过 80% 时，强制清理
            if (usagePercent > 80) {
            ...
            // 策略3: 内存降下来后恢复标志
            if (usagePercent < 50 && danmakuDensityReduced) {
                danmakuDensityReduced = false;
            }
            
        } catch (e) {
            console.error('内存监控出错:', e);
        }
    }, 30000); // 每30秒检查一次
...
// 减少弹幕密度
function reduceDanmakuDensity() {
    if (!art || !art.plugins.artplayerPluginDanmuku) return;
    
    const danmukuPlugin = art.plugins.artplayerPluginDanmuku;
    const currentDanmu = currentDanmuCache.danmuList || [];
    
    if (currentDanmu.length === 0) return;
    
    // 只保留50%的弹幕
    const reducedDanmu = currentDanmu.filter((_, index) => index % 2 === 0);
    
    danmukuPlugin.clear();
    danmukuPlugin.config({ danmuku: reducedDanmu });
    danmukuPlugin.load();
    
    showToast('内存优化：已减少弹幕密度', 'info');
}
...
function emergencyMemoryCleanup() {
    ...
    // 3. 清理 localStorage 中的大对象
    try {
        const keys = Object.keys(localStorage);
        let cleanedCount = 0;
        
        keys.forEach(key => {
            if (key.startsWith('anime_') || key.startsWith('danmu_') || key.startsWith('title_')) {
                try {
        ...
        console.log(`🧹 已清理 ${cleanedCount} 个缓存项`);
    } catch (e) {
    ...
    showToast('已执行内存清理，如仍卡顿请刷新页面', 'warning');
}
...
async function fetchDanmaku(episodeId, episodeIndex) {
    ...
    const totalCount = danmakuList.length;
    
    // 直接缓存完整列表
    currentDanmuCache = {
        episodeIndex: episodeIndex,
        danmuList: danmakuList,
        timestamp: Date.now()
    };
    
    console.log(`✅ 已缓存第${episodeIndex + 1}集弹幕（${totalCount}条）`);
    return danmakuList;
}

// 获取动漫集数信息
async function getAnimeEpisodes(animeId, cleanTitle) {
    try {
        ...
        animeDetailCache[detailCacheKey] = cacheEntry;
        ...
        while (attemptCount < MAX_ATTEMPTS) {
			...
			if (freshEpisodes) {
				...
				if (newMatch && newMatch.episodeId !== episodeId) {
					...
					const retryResult = await fetchDanmaku(newMatch.episodeId, episodeIndex);
					if (retryResult !== null) {
						return retryResult;
					}
				}
    ...
    if (!timers.autoCleanup) {
        timers.autoCleanup = setInterval(() => {
            ...
            if (detailKeys.length > 50) {
                const now = Date.now();
                const expired = detailKeys.filter(key => {
                    const item = animeDetailCache[key];
                    return item && (now - item.timestamp) > DANMU_CONFIG.cacheExpiration.detailCache;
                });
                
                expired.forEach(key => delete animeDetailCache[key]);
                saveCache(animeDetailCache);
                console.log(`🧹 已清理 ${expired.length} 个过期详情缓存`);
            }
            
			if (currentDanmuCache.timestamp > 0) {
				const cacheAge = Date.now() - currentDanmuCache.timestamp;
            ...
            try {
                const usage = new Blob([localStorage.getItem('animeDetailCache') || '']).size;
                if (usage > 4 * 1024 * 1024) {
                    console.warn('⚠️ localStorage 缓存过大，执行清理');
...
function initPlayer(videoUrl) {
    ...
    const hlsConfig = {
		...
		backBufferLength: 60,        // ✅ 从 90 改为 60
		maxBufferLength: 20,         // ✅ 从 30 改为 20
		maxMaxBufferLength: 40,      // ✅ 从 60 改为 40
		maxBufferSize: 20 * 1000 * 1000,  // ✅ 从 30MB 改为 20MB
        maxBufferHole: 0.5,
			...
			m3u8: function (video, url) {
                ...
                hls.on(Hls.Events.FRAG_BUFFERED, () => {
                    ...
                    try {
                        ...
                        // 策略 2: 紧急情况强制清理（防止崩溃）
                        else if (bufferAhead > 600) {
                            const cleanEnd = Math.max(0, current - 60);
        ...
        if (DANMU_CONFIG.enabled && art.plugins.artplayerPluginDanmuku) {
            const loadDanmaku = async () => {
                try {
                    ...
                    // ✅ 智能弹幕过滤：只保留有效范围的弹幕
					if (danmuku.length > 5000) {
						console.warn(`⚠️ 弹幕数量过多 (${danmuku.length}条)，启用智能过滤`);
						
						// 按时间排序并去重
						const uniqueDanmuku = [];
						const timeSet = new Set();
						
						danmuku.sort((a, b) => a.time - b.time).forEach(d => {
							const timeKey = Math.floor(d.time * 10); // 0.1秒精度
							if (!timeSet.has(timeKey) || uniqueDanmuku.length < 3000) {
								uniqueDanmuku.push(d);
								timeSet.add(timeKey);
							}
						});
						
						console.log(`✅ 过滤后保留 ${uniqueDanmuku.length} 条弹幕`);
						return uniqueDanmuku;
					}
...
function cleanCurrentVideoCache() {
    try {
        ...
        delete animeDetailCache[`title_${titleHash}`];
        if (currentDanmuAnimeId) {
            delete animeDetailCache[`anime_${currentDanmuAnimeId}`];
        }
        saveCache(animeDetailCache);
