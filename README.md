...
- **M 键**: 静音/取消静音
- **F 键**: 全屏/退出全屏
- **Esc 键**: 退出当前状态，如关闭面板 / 退出全屏
- **Alt + 左箭头**: 上一集
- **Alt + 右箭头**: 下一集

当输入框、搜索框、`textarea`、`select` 或 `contenteditable` 元素聚焦时，播放器快捷键会自动让出输入操作。

## 📱 移动端 / iPad 触屏手势

触屏设备（包括 iPhone、Android 手机、iPad、Android 平板，以及 iPadOS Safari 伪装成 `Macintosh` 的场景）支持以下快捷操作：

- **单击**: 显示/隐藏控制栏
- **双击**: 播放/暂停
- **长按**: 固定 3 倍速播放，松手恢复原倍速
- **左右滑动**: 调节播放进度，松手后再提交 seek
- **左半屏上下滑**: 调节亮度（使用播放器内亮度遮罩模拟）
- **右半屏上下滑**: 调节音量
- **屏幕边缘滑动**: 优先保留系统返回手势，不触发进度调节
